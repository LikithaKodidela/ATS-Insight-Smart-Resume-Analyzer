            ...
            if submitted:
                if not email or not password:
                    ...
                    if "error" in result:
                        ...
                        # Helpful hint for common Google OAuth conflict
                        if "invalid" in result["error"].lower() or "credentials" in result["error"].lower():
                            st.caption("💡 If you signed up with Google, use the 'Continue with Google' button below instead.")
                    else:
                        st.session_state.access_token  = result["access_token"]
                        st.session_state.refresh_token = result["refresh_token"]
                        st.session_state.user_id       = result["user_id"]
