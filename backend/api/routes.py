...
async def health_check(request: Request):
    ...
    return {
        ...
        'model_info':      getattr(request.app.state, 'model_info', {}),
    }
...
async def generate_pdf(
    ...
    from backend.services.pdf_generator import generate_pdf as _gen_pdf
    from fastapi.responses import Response

    try:
        pdf_bytes = _gen_pdf(data.model_dump())
        return Response(
            content=pdf_bytes,
...
async def generate_history_pdf(
    ...
    try:
        pdf_bytes = _gen_pdf(analysis_data)
        return Response(
            content=pdf_bytes,
            media_type="application/pdf",
        ...
        logger.error(f'Failed to generate PDF for history: {e}', exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to generate PDF: {e}")
