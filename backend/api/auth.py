...
from backend.core.config import DEV_AUTH_BYPASS, SUPABASE_JWT_SECRET, SUPABASE_URL
...
def _verify_token(token: str) -> dict:
    ...
    if alg == 'HS256':
        # Prefer the explicit JWT secret if configured (from Supabase Dashboard > Settings > API > JWT Secret)
        if SUPABASE_JWT_SECRET and SUPABASE_JWT_SECRET.strip() not in ('', 'your-supabase-jwt-secret-here'):
            return jwt.decode(
                token,
                SUPABASE_JWT_SECRET,
                algorithms=['HS256'],
        ...
        # Fallback: try JWKS endpoint — Supabase exposes the key there too
        jwks_client = _get_jwks_client()
        if jwks_client is not None:
            try:
                ...
                logger.warning(f'JWKS fallback for HS256 token failed: {exc}')
        raise jwt.InvalidTokenError(
            'HS256 token received but SUPABASE_JWT_SECRET is not configured. '
            'Add SUPABASE_JWT_SECRET to backend/.env (find it in Supabase Dashboard > Settings > API > JWT Secret).'
        )
...
def get_current_user(
    ...
    if DEV_AUTH_BYPASS:
        logger.warning('DEV_AUTH_BYPASS enabled; using local development user')
