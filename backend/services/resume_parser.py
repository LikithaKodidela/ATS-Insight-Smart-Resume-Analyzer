...
def _detect_mime_type(file_data: bytes, filename: str) -> str:
    """
    Detect MIME type using file header bytes (no C library required).
    Falls back to filename extension if headers are inconclusive.
    """
    # PDF: starts with %PDF
    if file_data[:4] == b'%PDF':
        return 'application/pdf'

    # DOCX (Office Open XML) and DOC: both start with PK zip header
    if file_data[:2] == b'PK':
        # DOCX is a zip archive
        return 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'

    # Legacy .doc (Compound Document File Format): D0 CF 11 E0
