...
def calculate_semantic_similarity(
    ...
    # Use normalize_embeddings=True so the model's L2-normalised embedding
    # space is used — this matches the CosineSimilarityLoss training objective
    # of the fine-tuned all-MiniLM-L6-v2 model.  For unit vectors, dot product
    # equals cosine similarity directly (no division needed).
    resume_emb = embedder.encode(
        resume_text[:5000], convert_to_tensor=False, normalize_embeddings=True
    )
    jd_emb = embedder.encode(
        jd_text[:5000], convert_to_tensor=False, normalize_embeddings=True
    )
